...
export const tankDataSchema = z.object({
  ...
  tankStatus: z.enum(
    ["Empty", "Low", "Normal", "High", "Full", "Sensor Error"],
    {
      errorMap: () => ({
        message: "tankStatus must be Empty, Low, Normal, High, Full, or Sensor Error.",
      }),
    }
  ),
});
...
export const ultrasonicReadingSchema = z.object({
  deviceId: z
    .string()
    .trim()
    .min(1, "deviceId must be a non-empty string."),
  ...
  // YF-S201 flow sensor binary detection
  waterFlowDetected: z.boolean().nullable().optional(),

  // Electricity source classification (DAWLE = grid, MOTEUR = generator)
